...
captureParams(std::multimap<std::string, std::string>& requirement)
{
  ...
  for (auto& item : requirement) {
    ...
    // get the base 64 here, then do the base 64 decoding and pass the binary
    // check std::get<0>(item) to make sure it is eqaul to "issued-cert" then decode
    std::get<1>(item) = captured;
    if(std::get<0>(item) == "issued-cert"){
      std::get<1>(item) = base64_decode(captured);
      std::cerr << "length: " << std::get<1>(item).length() << std::endl;
    }
  }
